...
import marcclaessens.galgje.WordLists;
import marcclaessens.galgje.sound.FileSound;
import marcclaessens.galgje.sound.ResourceSound;
import marcclaessens.galgje.sound.Sound;
import marcclaessens.galgje.sound.SoundFormat;
import marcclaessens.galgje.sound.SoundType;
import marcclaessens.galgje.ui.KeyboardLayout;
import marcclaessens.galgje.ui.KeyboardLayoutReader;
...
public class OptionsPanel extends JPanel {
	...
	private void initSoundGroup(ButtonGroup group, JRadioButton sndOff, JRadioButton sndDefault, JRadioButton sndCustom,
			SoundType type) {
		group.add(sndOff);
	...
	public void loadPrefs() {
		...
		if (prefWord != null && !prefWord.equals("") && !prefWord.equals(wordList.getSelectedItem())) {
			wordList.setSelectedItem(prefWord);
			wordList.getEditor().setItem(prefWord);
		}
		wordList.actionPerformed(new ActionEvent(wordList, 0, null)); // load new word in the selected language

		String prefLaf = prefs.get(PREFS_LAF, null);
		if (prefLaf != null && !prefLaf.equals("") && !prefLaf.equals(lafList.getSelectedItem())) {
			lafList.setSelectedItem(prefLaf);
			lafList.getEditor().setItem(prefLaf);
			lafList.actionPerformed(new ActionEvent(lafList, 0, null));
		}
		String prefKeyboard = prefs.get(PREFS_KEYBOARD, null);
		if (prefKeyboard != null && !prefKeyboard.equals("")
				&& !prefKeyboard.equals(keyboardNameList.getSelectedItem())) {
			keyboardNameList.setSelectedItem(prefKeyboard);
			keyboardNameList.getEditor().setItem(prefKeyboard);
		...
		fontSizeList.getEditor().setItem(prefFontsize);
	...
	private void loadPrefSound(String guessSound, JRadioButton sndOff, JRadioButton sndDefault,
			JRadioButton sndCustom) {
	...
	private void initWordList() {
		...
		wordList.getEditor().setItem(wordList.getSelectedItem());
	...
	private void initLafList() {
		...
		lafList.getEditor().setItem(lafList.getSelectedItem());
	...
	private void initFontSizeList() {
		...
		fontSizeList.getEditor().setItem(fontSizeList.getSelectedItem());
	...
	private void initKeyboardNameList() {
		...
		keyboardNameList.getEditor().setItem(keyboardNameList.getSelectedItem());
